...
# navigate between them. It does this by running an AWK script (which does the real work) on every HTML file (except the
# the very top level `index.html` index). The AWK script replaces the (Doxygen or LCOV) HTML pages' project version with
...
find "${1:-.}" -mindepth 2 -name '*.html' -print0 | xargs -0 -n 1000 -P 8 -r gawk -f '.selector' -i 'inplace' --
